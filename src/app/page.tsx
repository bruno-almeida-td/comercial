      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Controle de vouchers Tax Summit 2026
          </p>
        </div>
        <button
          onClick={loadData}
        ...
        <>
          ...
          {data.total > 0 && (
            <div className="mt-8 card">
              <h2 className="text-lg font-semibold mb-4">Utilização de Vouchers</h2>
              <div className="w-full bg-gray-100 rounded-full h-6 overflow-hidden">
                <div className="h-full flex">
                  <div
                    className="bg-green-500 transition-all duration-500"
                    style={{
                      width: `${(data.entregues / data.total) * 100}%`,
                    }}
                    title={`Entregues: ${data.entregues}`}
                  />
                  <div
              ...
              <div className="flex gap-6 mt-3 text-sm">
                ...
                <span className="flex items-center gap-2">
                  ...
                  Livres ({Math.max(0, data.restantes - data.reservados)})
                </span>
              </div>
            </div>
          )}
